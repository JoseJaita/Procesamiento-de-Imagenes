...
int main(int argc, char *argv[]){
    ...
    //VideoCapture video("others/videos_final/PS3_chess.avi");
    VideoCapture video("others/videos_final/LifeCam_chess.avi");
    ...
    if (r && false) {
      ...
      while (1) {
        ...
        cout << "frame: "<<endl;
        vector<Point2f> points2D,points2D1;
        Mat g,g1;
        ...
        bool b1 = findChessboardCorners(g1, pattern_size, points2D1,
                                       CALIB_CB_ADAPTIVE_THRESH +CALIB_CB_NORMALIZE_IMAGE + CALIB_CB_FAST_CHECK);
        if (b) calibration.drawCentersAndRect(image_undistorted,points2D);
        if (b1) calibration.drawCentersAndRect(image_undistorted1,points2D1);
