...
bool Refinement::canonical(const Mat& src, Mat& dst, Mat& H ,int idx){
  Mat gray;
  ...
  case RING:{
    ...
    int w = int(1.5*s.width);
    int h = int(1.5*s.height);
    if (w>= src.cols) w = s.width;
    if (h>= src.rows) h = s.height;
    //cout << idx << " "<<calibration.getThresholdIdx(idx)<<" "<<w<<" "<<h<<endl;
...
void Refinement::normalizePoints(const vector<Point2f>& src, vector<Point2f>& dst){
  Size pattern_size = calibration.getPatternSize();
  Size img_size = calibration.getImageSize();
  float length = calibration.getLength();

  if (pattern_type == CIRCLE){
    dst.clear();
    for (auto p : src){
      p.x *= 2.*length*(pattern_size.width+1)/float(img_size.width);
      p.x -= length;
      p.y *= length*(pattern_size.height+1)/float(img_size.height);
      dst.push_back(p);
    }
  ...
  dst.clear();
  for (auto p : src){
    p.x *= length*(pattern_size.width+1)/float(img_size.width);
    p.y *= length*(pattern_size.height+1)/float(img_size.height);
    dst.push_back(p);
  }
  }

  //for(int i = 0;i<src.size();i++) cout << src[i]<<" "<<dst[i]<<endl;
}

void Refinement::unProject(const Mat& src, Mat& dst, Mat& H,Mat A, Mat rotate, Mat translate){
...
string Refinement::processing(){
  ...
  for (int n_iter = 0; n_iter<5; n_iter++){
    ...
    //for(int h = 0;h<rvects.size();h++) cout <<h<<endl<<rvects[h] << endl;
    ...
    for(int i=0; i < n_frames; i++){
      Mat rotate, translate, img_src, H_c, H_p;
      ...
      unProject(img_undistord, img_unproject, H_p,A, rotate, translate);
      bool r = canonical(img_unproject, img_canonical, H_c,idx_images[i]);
      if (!r) {cout << i<<endl;continue;}


      vector<Point2f> new_points2D;
      ...
      if (pattern_type == CHESSBOARD)
        ...
        */
        ...
        vector<Point2f> points_c_norm;
        r = findPointsCanonical(img_canonical, points_canonical);
        //r = findPointsCanonical(img_unproject, points_c_norm);
        if (!r) continue;
        //normalizePoints(points_canonical, points_c_norm);
        perspectiveTransform(points_canonical,points_c_norm,H_c.inv());

        vector<Point2f> new_points2D_d;
        perspectiveTransform(points_c_norm, new_points2D_d,H_p.inv());

        //vector<Point2f> new_points2D;
        distortCenters(new_points2D_d,new_points2D);
      ...
      idx_images_good.push_back(idx_images[i]);

    }
    // new calibration
    cout << "Calibration number: "<<n_iter+1<<endl;
    bool r = calibration.processing();
    if (!r) break;
    calibration.showResults();

    idx_images = idx_images_good;
    for(auto a : idx_images) cout <<a<<" ";
    cout << endl;
  }
  //save new caibration
...
bool Refinement::testCanonical(const Mat& src, Mat& dst, Mat& cambio_centers,Engine& egn,Calibration cal){
  ...
  */
  ...
  unProject(img_undistord, img_unproject, H_p,A, rotate, translate);
  Mat H_c;
  bool r = canonical(img_unproject, dst,H_c,-1);
